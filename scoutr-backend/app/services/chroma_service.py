...
class ChromaService:
    ...
    def search_players(self, query: dict):
        ...
        if query.get("position"):
            # Map simplified query positions (e.g. "right-back") to acceptable 
            # StatsBomb dataset values, which are lowercase with spaces or variations
            pos = query["position"].lower().replace("-", " ")
            if "back" in pos:
                # E.g., user asks for "right-back", statsbomb uses "right wing back" or "right back"
                # Chroma's $in syntax allows multiple possible string matches.
                if "right" in pos:
